...
def main():
    ...
    client = NodeExporterClient("http://localhost:9100/metrics")
    ...
    try:
        ...
        print(f"CPU Usage: {all_metrics['cpu']['usage_percent']}%")
        
        if all_metrics['memory']:
            mem = all_metrics['memory']
            print(f"Memory: {mem['used_percent']}% ({mem['used_bytes']//1024//1024} MB / {mem['total_bytes']//1024//1024} MB)")
        
        if all_metrics['disk']:
            disk = all_metrics['disk']
            print(f"Disk: {disk['used_percent']}% ({disk['used_bytes']//1024//1024//1024} GB / {disk['total_bytes']//1024//1024//1024} GB)")
        
        if all_metrics['load']:
            load = all_metrics['load']
            print(f"Load Average: {load.get('load_1min', 'N/A')} {load.get('load_5min', 'N/A')} {load.get('load_15min', 'N/A')}")
